<script lang="ts">
	...
	async function handleDownload(track: PlayableTrack, event?: MouseEvent) {
		...
		if (isSonglinkTrack(track)) {
			if (track.tidalId) {
				...
				alert('Esta pista debe reproducirse primero antes de descargarse. Haz clic para reproducirla y luego descárgala.');
				return;
			}
		} else {
			trackId = track.id;
		...
		if (!Number.isFinite(trackId) || trackId <= 0) {
			...
			alert('No se puede descargar esta pista - ID no válido');
			return;
		}
		...
		try {
			...
			if (error instanceof DOMException && error.name === 'AbortError') {
				...
				const fallbackMessage = 'Error al descargar la pista. Por favor intenta de nuevo.';
		...
		try {
			...
			const message =
				err instanceof Error && err.message
					? err.message
					: 'Error al descargar el álbum. Por favor intenta de nuevo.';
	...
	async function handleUrlImport() {
		...
		try {
			...
			searchStore.error = err instanceof Error ? err.message : 'Error al importar desde URL';
	...
	async function handleSearch() {
		...
		try {
			...
			searchStore.error = err instanceof Error ? err.message : 'Búsqueda fallida';
	...
	async function handleStreamingUrlConversion() {
		...
		try {
			...
			if (!tidalInfo) {
				searchStore.error = `No se pudo encontrar el equivalente en TIDAL para este enlace de ${platformName || 'streaming'}. Es posible que el contenido no esté disponible en TIDAL.`;
				searchStore.isLoading = false;
				return;
			}
			...
			searchStore.error = err instanceof Error ? err.message : 'Error al convertir URL';
	...
	async function handleSpotifyPlaylistConversion() {
		...
		searchStore.error = null;
		searchStore.playlistLoadingMessage = 'Cargando playlist...';
		...
		try {
			...
			if (!spotifyTrackUrls || spotifyTrackUrls.length === 0) {
				searchStore.error =
					'No se pudieron obtener pistas de la playlist de Spotify. La playlist podría estar vacía o ser privada.';
			...
			searchStore.playlistLoadingMessage = `Cargando ${spotifyTrackUrls.length} pistas...`;
			...
			results.forEach((result, index) => {
				...
				searchStore.playlistLoadingMessage = `Cargadas ${index + 1}/${spotifyTrackUrls.length} pistas...`;
			});
			...
			if (searchStore.tracks.length === 0) {
				searchStore.error = 'No se pudieron encontrar equivalentes en TIDAL para ninguna pista de esta playlist.';
				searchStore.playlistLoadingMessage = null;
				searchStore.isPlaylistConversionMode = false;
				return;
			}
			...
			if (failedTracks.length > 0) {
				...
				searchStore.playlistLoadingMessage = `Cargadas ${searchStore.tracks.length} pistas (${failedTracks.length} fallidas)`;
			} else {
				searchStore.playlistLoadingMessage = `¡Cargadas exitosamente ${searchStore.tracks.length} pistas!`;
			}
			...
			searchStore.error = err instanceof Error ? err.message : 'Error al cargar playlist de Spotify';
...
<div class="w-full">
						...
						placeholder={isQueryATidalUrl
							...
							: isQueryASpotifyPlaylist
								...
								: isQueryAStreamingUrl
									...
									: 'Buscar pistas, álbumes, artistas... o pegar una URL'}
					...
					{#if false} <!-- hide the region selector that doesn't even work lol -->
						<div class="relative w-auto">
							...
							<select
								...
								title="Cambiar región de búsqueda"
							>
	...
	{#if !isQueryAUrl}
		<div class="scrollbar-hide mb-6 flex gap-2 overflow-x-auto border-b border-gray-700">
			...
			>
				<Music size={18} />
				<span class="text-sm sm:text-base">Pistas</span>
			</button>
			<button
			...
			{#if searchStore.isPlaylistConversionMode}
				<div class="mb-6 flex flex-wrap items-center gap-3">
					...
					<div class="ml-auto text-sm text-gray-400">
						{searchStore.tracks.length} de {searchStore.playlistConversionTotal} pistas
					</div>
				</div>
			{/if}
						...
						{:else}
							...
							<div class="flex items-center gap-2 text-sm text-gray-400">
								<button
									...
									class="rounded-full p-2 text-gray-400 transition-colors hover:text-white"
									title={downloadingIds.has(track.id) ? 'Cancelar descarga' : 'Descargar pista'}
									aria-label={downloadingIds.has(track.id)
										? `Cancelar descarga para ${track.title}`
				...
				{#each searchStore.albums as album}
					<div class="group relative text-left">
						<button
							...
							aria-label={`Descargar ${album.title}`}
						>
							{#if albumDownloadStates[album.id]?.downloading}
			...
			<div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
				{#each searchStore.playlists as playlist}
					...
					>
						...
						<p class="text-xs text-gray-500">{playlist.numberOfTracks} pistas</p>
					</a>
			{/each}
			</div>
			<!-- News Section -->
