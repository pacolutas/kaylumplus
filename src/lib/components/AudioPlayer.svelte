<script lang="ts">
	...
	async function loadTrack(track: PlayableTrack) {
		...
		// Always load standard track with 'LOW' quality (AAC 96kbps)
		const requestedQuality: AudioQuality = 'LOW';
		await loadStandardTrack(tidalTrack, requestedQuality);
							...
							<div class="flex items-start gap-3">
								<div class="min-w-0 flex-1">
									...
									{#if $ffmpegBanner.phase === 'countdown'}
										<p class="mt-1 text-xs text-blue-100/80">
											Iniciando en {$ffmpegBanner.countdownSeconds} segundos...
										</p>
									{:else if $ffmpegBanner.phase === 'loading'}
										<p class="mt-1 text-xs text-blue-100/80">
											Preparando codificador... {formatPercent($ffmpegBanner.progress)}
										</p>
									{:else if $ffmpegBanner.phase === 'ready'}
										<p class="mt-1 text-xs text-blue-100/80">FFmpeg está listo para usar.</p>
									{:else if $ffmpegBanner.phase === 'error'}
										<p class="mt-1 text-xs text-red-200">
											{$ffmpegBanner.error ?? 'Error al cargar FFmpeg.'}
										</p>
									{/if}
								</div>
								{#if $ffmpegBanner.dismissible}
									<button
										...
										aria-label="Descartar descarga de FFmpeg"
									>
										<X size={16} />
									</button>
								{/if}
							...
							<div class="flex items-start gap-3">
								...
								<button
									...
									aria-label={task.cancellable
										? `Cancelar descarga de ${task.title}`
										: `Descartar descarga de ${task.title}`}
								>
									<X size={16} />
								</button>
							</div>
				...
				{#if $playerStore.currentTrack}
										...
										<p class="text-xs text-gray-500">
											...
											{#if currentPlaybackQuality && asTrack($playerStore.currentTrack).audioQuality && currentPlaybackQuality !== asTrack($playerStore.currentTrack).audioQuality}
												...
												<span class="text-gray-500">
													({formatQualityLabel(asTrack($playerStore.currentTrack).audioQuality)} disponible)
												</span>
											{/if}
											{#if bitDepthLabel}
												<span class="mx-1 text-gray-600" aria-hidden="true">•</span>
							...
							<div class="flex items-center gap-1 sm:gap-2">
								...
								<button
									...
									class="player-toggle-button p-1.5 sm:p-2 {$lyricsStore.open
										...
										: ''}"
									aria-label={$lyricsStore.open ? 'Ocultar letras' : 'Mostrar letras'}
									aria-expanded={$lyricsStore.open}
									type="button"
								>
									<ScrollText size={16} class="sm:w-[18px] sm:h-[18px]" />
									<span class="hidden sm:inline">Letras</span>
								</button>
								<button
									...
									class="player-toggle-button p-1.5 sm:p-2 {showQueuePanel
										? 'player-toggle-button--active'
										: ''}"
									aria-label="Alternar panel de cola"
									aria-expanded={showQueuePanel}
									type="button"
								>
									<ListMusic size={16} class="sm:w-[18px] sm:h-[18px]" />
									<span class="hidden sm:inline">Cola ({$playerStore.queue.length})</span>
								</button>
							</div>
						...
						>
							<div class="flex items-center justify-between gap-2">
								<div class="flex items-center gap-2 text-gray-300">
									<ListMusic size={18} />
									<span class="font-medium">Cola de Reproducción</span>
								...
								<div class="flex items-center gap-2">
									...
									<button
										...
										aria-label="Cerrar panel de cola"
									>
										<X size={16} />
									</button>
								</div>
										...
										<li>
											...
											>
												...
												<button
													...
													aria-label={`Eliminar ${queuedTrack.title} de la cola`}
													type="button"
												>
													<X size={14} />
												</button>
					...
					{#if $playerStore.currentTrack && $playerStore.isLoading}
						<div class="loading-overlay">
							...
							<p class="text-sm font-medium text-gray-200">Cargando pista...</p>
						</div>
					{/if}
				{:else}
