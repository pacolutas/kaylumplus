<script lang="ts">
	...
	const QUALITY_OPTIONS: Array<{ value: AudioQuality; label: string; description: string; disabled?: boolean }> = [
		{
			...
			description: 'Streaming AAC 96kbps'
		}
	];
	...
	const PERFORMANCE_OPTIONS: Array<{
	...
	}> = [
		{
			value: 'medium',
			label: 'Equilibrado',
			description: 'Animaciones fluidas con efectos visuales'
		},
		{
			value: 'low',
			label: 'Rendimiento',
			description: 'Efectos mínimos para mejor rendimiento'
		}
	];

	const playbackQualityLabel = $derived(() => {
		const quality = $playerStore.quality;
		// Since we only support LOW, just return the label for it mostly, or fallback
		return QUALITY_OPTIONS.find((option) => option.value === quality)?.label ?? 'Calidad';
	});
	...
	const navigationMessage = $derived(() => {
		...
		if (!primarySegment) return 'Cargando';
	...
	$effect(() => {
		...
		if (track) {
			...
			document.title = `${prefix} ${title} • ${artist} | Kaylum Plus`;
		} else {
	...
	async function handleExportQueueCsv(): Promise<void> {
		...
		if (tracks.length === 0) {
			showSettingsMenu = false;
			alert('Añade pistas a la cola antes de exportar.');
			return;
		}
	...
	async function downloadQueueIndividually(tracks: PlayableTrack[], quality: AudioQuality): Promise<void> {
		...
		try {
				...
				try {
					...
					const message =
						error instanceof Error && error.message
							? error.message
							: 'Error al descargar la pista. Por favor intenta de nuevo.';
			...
			if (errors.length > 0) {
				const summary = [
					'No se pudieron descargar algunas pistas individualmente:',
					...errors.slice(0, 3),
					errors.length > 3 ? `…y ${errors.length - 3} más` : undefined
				]
					.filter(Boolean)
					.join('\n');
				alert(summary);
	...
	async function handleQueueDownload(): Promise<void> {
		...
		if (tracks.length === 0) {
			showSettingsMenu = false;
			alert('Añade pistas a la cola antes de descargar.');
			return;
		}
					...
					<div class="settings-trigger" bind:this={settingsMenuContainer}>
						...
						>
							<span class="toolbar-button__label">
								<Settings size={16} />
								<span class="toolbar-button__text">Ajustes</span>
							</span>
							...
							>
								<div class="settings-grid">
									...
									<section class="settings-section settings-section--wide">
										...
										>
											<span class="glass-option__content">
												...
												<span class="glass-option__description">Aplica a descargas de 320kbps y 96kbps.</span>
											</span>
									...
									<section class="settings-section settings-section--bordered">
										<p class="section-heading">Acciones de cola</p>
											...
											>
												<span class="glass-action__label">
													...
													{:else if downloadMode === 'csv'}
														<FileSpreadsheet size={16} />
														<span>Exportar enlaces de cola</span>
													{:else}
														<Download size={16} />
														<span>Descargar cola</span>
													{/if}
										...
										<p class="section-footnote">
											Las acciones de cola siguen su selección anterior. Los paquetes ZIP requieren al menos dos pistas,
											mientras que las exportaciones CSV capturan los enlaces de las pistas sin descargar audio.
										</p>
									</section>
								</div>
							</div>
